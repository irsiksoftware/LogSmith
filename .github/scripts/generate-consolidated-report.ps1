...
# Collect all test cases across all versions
$allTests = @{}  # Key: test name, Value: @{ "2022.3" = @{Result, Duration, Mode}, ... }

foreach ($version in $versions) {
    $versionLabel = $versionLabels[$version]

    # Process EditMode tests
    $editXmlPath = "$ArtifactsPath/Test-Results-EditMode-$version-Windows/EditMode-results.xml"
    if (Test-Path $editXmlPath) {
        [xml]$editXml = Get-Content $editXmlPath
        $testCases = $editXml.SelectNodes("//test-case")

        foreach ($test in $testCases) {
            $testName = $test.fullname
            if (-not $allTests.ContainsKey($testName)) {
                $allTests[$testName] = @{}
            }
            $allTests[$testName][$versionLabel] = @{
                Result = $test.result
                Duration = [double]$test.duration
                Mode = "EditMode"
                FailureMessage = if ($test.failure) { $test.failure.message } else { $null }
            }
        }
    }

    # Process PlayMode tests
    $playXmlPath = "$ArtifactsPath/Test-Results-PlayMode-$version-Windows/PlayMode-results.xml"
    if (Test-Path $playXmlPath) {
        [xml]$playXml = Get-Content $playXmlPath
        $testCases = $playXml.SelectNodes("//test-case")
...
$overallTotal = $allTests.Count
$overallPassed = 0
$overallFailed = 0
$overallSkipped = 0

foreach ($testData in $allTests.Values) {
    # Count based on first available version result
    $firstResult = ($testData.Values | Select-Object -First 1).Result
    switch ($firstResult) {
...
$status = if ($overallFailed -eq 0) { "PASSED" } else { "FAILED" }
$statusColor = if ($overallFailed -eq 0) { "#4CAF50" } else { "#f44336" }
$statusGradient = if ($overallFailed -eq 0) { "#45a049" } else { "#da190b" }

# Build test table HTML
$testTableHtml = ""
$sortedTests = $allTests.GetEnumerator() | Sort-Object { $_.Key }

foreach ($entry in $sortedTests) {
    $testName = [System.Security.SecurityElement]::Escape($entry.Key)
    $testData = $entry.Value

    # Determine row class based on overall result
    $hasFailure = $false
    $hasSkipped = $false
    foreach ($vData in $testData.Values) {
        if ($vData.Result -eq "Failed") { $hasFailure = $true }
        if ($vData.Result -eq "Skipped") { $hasSkipped = $true }
    }

    $rowClass = if ($hasFailure) { "test-failed" } elseif ($hasSkipped) { "test-skipped" } else { "test-passed" }

    $testTableHtml += "<tr class='$rowClass'>`n"
    $testTableHtml += "  <td class='test-name'>$testName</td>`n"
...
<head>
  ...
  <title>LogSmith - All Versions Test Report #$RunNumber</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      <h1>LogSmith - All Unity Versions Test Report</h1>
    ...
    <div class="meta">
      ...
      <div class="meta-item">
        <span class="meta-label">Unity Versions</span>
        <span class="meta-value">2022.3, 2023.2, 6000.0</span>
      </div>
      <div class="meta-item">
    ...
    <div class="summary">
      <div class="stat-box total">
        <div class="stat-number">$overallTotal</div>
        <div class="stat-label">Total Tests</div>
      </div>
      <div class="stat-box passed">
        <div class="stat-number">$overallPassed</div>
        <div class="stat-label">Passed</div>
      </div>
      <div class="stat-box failed">
        <div class="stat-number">$overallFailed</div>
        <div class="stat-label">Failed</div>
      </div>
      <div class="stat-box skipped">
        <div class="stat-number">$overallSkipped</div>
        <div class="stat-label">Skipped</div>
      </div>
    </div>

    <div class="content">
      <h2>Test Results by Unity Version ($overallTotal unique tests)</h2>
      <table>
        <thead>
          <tr>
            <th>Test Name</th>
...
Write-Host "Consolidated HTML report generated: $HtmlPath"
Write-Host "Total unique tests: $overallTotal"
Write-Host "Passed: $overallPassed | Failed: $overallFailed | Skipped: $overallSkipped"
